...
_INJECTION_PATTERNS = [
    ...
    r"(?:reveal|print|show|repeat|output|leak|dump)\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions|initial\s+prompt|context|rules)",
    r"(?:bypass|disable|ignore)\s+(?:safety|guardrails?|content\s+filter|guidelines|policies)",
    r"you\s+are\s+now\s+(?:a\s+)?(?:dan|evil|unfiltered|uncensored|jailbreak)",
    r"(?:developer\s+mode|unrestricted\s+mode|god\s+mode)",
    r"pretend\s+(?:to\s+be|you\s+are)\s+(?:an?\s+)?(?:unfiltered|unrestricted|evil|ai\s+without\s+rules)",
    ...
    r"(?:write|compose|generate)\s+(?:an?\s+)?(?:poem|rap|joke|riddle|song|lyrics|essay|story|novel|speech|tweet)",
    r"(?:write|create|debug|fix|generate)\s+(?:a\s+)?(?:python|javascript|cpp|java|c#|sql|bash|html|css|code|script|program|app)",
    r"solve\s+(?:this\s+)?equation",
    r"(?:how\s+to\s+)?(?:hack|exploit|bypass|ddos|crack|phish)",
    r"(?:make|manufacture|cook|synthesize)\s+(?:a\s+)?(?:bomb|explosive|weapon|poison|meth|drug)",
