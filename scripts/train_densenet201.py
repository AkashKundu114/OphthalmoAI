"""
DenseNet-201 Retinal Fundus Classifier Training Script.
Densely connected convolutional networks with maximal feature reuse,
alleviating vanishing gradients and strengthening feature propagation.
...
- Exhaustive Hardware Telemetry (CPU, GPU, RAM, VRAM, thermals)
"""
...
def get_densenet201(num_classes=NUM_CLASSES):
    model = models.densenet201(weights=models.DenseNet201_Weights.DEFAULT)
    model.classifier = nn.Linear(model.classifier.in_features, num_classes)
    return model

def parse_args():
    parser = argparse.ArgumentParser(description="Train DenseNet-201 on Retinal Fundus Dataset")
...
def main():
    ...
    print("=" * 70)
    print("DENSENET-201 RETINAL FUNDUS TRAINING")
    ...
    telemetry = HardwareTelemetry(use_gpu=(device.type == "cuda" and torch.cuda.is_available()), model_name=f"DenseNet-201_{args.precision}_bs{args.batch_size}")
    ...
    model = get_densenet201(NUM_CLASSES).to(device)
    ...
    out_ckpt = MODELS_DIR / f"densenet201_{args.precision}_bs{args.batch_size}.pth"
    best_f1 = 0.0
