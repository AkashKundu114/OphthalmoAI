...
| [`scripts/train_convnext_small.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/train_convnext_small.py) | **ConvNeXt-Small**: 7x7 Depthwise ConvNet | `cuda` / `cpu` | `fp16`, `bf16`, `fp32` | 32 |
| [`scripts/train_densenet201.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/train_densenet201.py) | **DenseNet-201**: Feature Reuse Network | `cuda` / `cpu` | `fp16`, `bf16`, `fp32` | 32 |
| [`scripts/train_resnet50.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/train_resnet50.py) | **ResNet-50**: Universal Baseline | `cuda` / `cpu` | `fp16`, `bf16`, `fp32` | 32 |
...
| [`scripts/train_ensemble_bf16.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/train_ensemble_bf16.py) | **BF16 Meta-Ensemble**: Tensor Core bfloat16 | `cuda` / `cpu` | `bf16` | 32 |
| [`scripts/train_ensemble_bs32.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/train_ensemble_bs32.py) | **BS32 Meta-Ensemble**: 8GB VRAM Optimized | `cuda` / `cpu` | `fp16` | 32 |
| [`scripts/run_all_trainings.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/run_all_trainings.py) | **Master Orchestrator**: Automated sweeps & smoke tests | Any | Any | Configurable |
| [`scripts/run_conformal_calib.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/run_conformal_calib.py) | **Conformal Calibration**: 99% emergency coverage | `cuda` / `cpu` | `fp32` | 32 |
| [`scripts/calibrate_models.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/calibrate_models.py) | **Temperature Scaling**: Platt ECE Minimizer | `cuda` / `cpu` | `fp32` | 32 |
| [`scripts/evaluate_models.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/evaluate_models.py) | **Evaluation Suite**: AUROC, ECE, Confusion Matrix | `cuda` / `cpu` | `fp32` | 32 |
| [`scripts/evaluate_ensemble.py`](file:///d:/AI-based%20Retinal%20Disease%20Predictor/scripts/evaluate_ensemble.py) | **Ensemble Evaluator**: Tri-Backbone Test Report | `cuda` / `cpu` | `fp32` | 32 |
...
```powershell
# 1. Train Production Target (EfficientNet-B4) with FP16 on GPU (Batch Size 32, 10 Epochs)
...
# 2. Train ConvNeXt-Small with BF16 on GPU (Batch Size 32, 10 Epochs)
...
# 3. Train DenseNet-201 with FP16 on GPU (Batch Size 32, 10 Epochs)
...
```powershell
# EfficientNet-B4 Dedicated Script
...
& "d:\AI-based Retinal Disease Predictor\venv\Scripts\python.exe" scripts\train_convnext_small.py --precision fp16 --batch-size 32 --epochs 10 --device cuda

# DenseNet-201 Dedicated Script
& "d:\AI-based Retinal Disease Predictor\venv\Scripts\python.exe" scripts\train_densenet201.py --precision fp16 --batch-size 32 --epochs 10 --device cuda
...
& "d:\AI-based Retinal Disease Predictor\venv\Scripts\python.exe" scripts\train_cpu_resnet50.py --batch-size 16 --epochs 5 --threads 32
```
